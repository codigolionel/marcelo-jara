...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 bg-background ${
      scrolled ? "shadow-sm py-1.5 border-b border-border/40" : "py-3"
    }`}>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className={`whitespace-nowrap text-[16px] font-semibold tracking-wide transition-all duration-300 flex items-center group ${
                scrolled ? "text-navy hover:text-[#C0392B]" : "text-navy hover:text-[#C0392B]"
              }`}
            >
